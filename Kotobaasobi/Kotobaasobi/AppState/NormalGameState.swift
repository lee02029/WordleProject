...
struct NormalGameState: Equatable {
    enum NormalGameState: Equatable {
        case playing
        case gameOver(didWin: Bool)
    }
    
    @BindableState var gameLanguage: GameLanguage = .NormalGameJapanese
    
    var word: String = Words.randomWord(in: .NormalGameJapanese)
    var currentGuess: String = ""
    var guesses: [String] = []
    var gameState: NormalGameState = .playing
...
enum NormalGameAction: BindableAction {
    case keyboardInput(KeyboardKey)
    case newGame
    case failGame
    case switchLanguage(GameLanguage)
    case binding(BindingAction<NormalGameState>)
}

struct NormalGameEnv {}

let NormalGameReducer = AnyReducer<NormalGameState, NormalGameAction, NormalGameEnv> { state, action, env in
    
    switch action {
    case let .keyboardInput(.character(char, _)):
        guard state.gameState == .playing else { return .none }
        if state.currentGuess.count < 4 {
    ...
    case .keyboardInput(.symbol("return.left")):
        guard state.currentGuess.count == 4 else { return .none }
...
extension NormalGameState {
    var boxes: [[LetterBox]] {
        ...
        if currentGuess.count > 0 {
            ...
            + Array(repeating: .empty, count: 4 - currentGuess.count)
            result.append(current)
        }
        
        return result + Array(repeating: Array(repeating: LetterBox.empty, count: 4), count: 5 - result.count)
    }
