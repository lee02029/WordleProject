...
protocol EasyKeyboardViewControllerDelegate: AnyObject {
    func EasykeyboardViewController(
        _ vc: EasyKeyboardViewController,
        didTapKey letter: Character
    )
}

class EasyKeyboardViewController: UIViewController {
    // MARK: - Properties
    
    weak var delegate: EasyKeyboardViewControllerDelegate?
    
    let letters = ["あいうも", "おかきく"]
    ...
    private func keyboardCollectionViewConstranints() {
        ...
        keyboardCollectionView.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: 30).isActive = true
        keyboardCollectionView.topAnchor.constraint(equalTo: view.topAnchor, constant: 40).isActive = true
        keyboardCollectionView.bottomAnchor.constraint(equalTo: view.bottomAnchor).isActive = true
    }
}


extension EasyKeyboardViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        var letter = keys[indexPath.section][indexPath.row]
        delegate?.EasykeyboardViewController(self,didTapKey: letter)
        print(letter)
        if letter == "a" {
            keys.popLast()
        }
